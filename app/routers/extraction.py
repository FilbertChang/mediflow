...
def extract_from_note(input: NoteInput, db: Session = Depends(get_db)):
...
def get_history(db: Session = Depends(get_db)):
    records = db.query(ExtractionHistory).order_by(ExtractionHistory.created_at.desc()).limit(20).all()
